...
var HotList = React.createClass({
    ...
    componentDidMount: function () {
        fetch(homeUrl)
            ...
            .then((responseData) => {
                this.setState({
                    title: responseData.body.block[0].blockname,
                    dataSource: this.state.dataSource.cloneWithRows(responseData.body.block[0].list),
                    loaded: true
                })
            });
    },
            ...
            <TouchableHighlight style={styles.item} onPress={()=>this.props.onPress(rowData)}>
                <View>
                    ...
                    <View style={styles.itemBottom}>
                        <Text style={styles.itemTitle}>{rowData.nameCn}</Text>
                        <Text style={styles.itemDesc}>{rowData.subTitle}</Text>
                    </View>
                </View>
            </TouchableHighlight>
        )
